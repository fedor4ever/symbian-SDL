...
void SetStatusCharAndTimer(TPtrC aValue)
{
	...
	current_video->hidden->iStatusChar.Copy(aValue);
}
...
MY_EXPORT_C char* CSDLApp::GetExecutablePathCStr()
{
 ...
 */
MY_EXPORT_C void CSDLApp::PreInitializeAppL()
{
}
 ...
 */
MY_EXPORT_C void CSDLApp::LaunchAppL(int, char** params)
{
#ifndef __DLL__
	main(1, params);
#endif
...
void CEBasicAppUi::UpdateScreenOffset(TInt aKeyCode)
	...
	switch (aKeyCode)
		...
		case EStdKeyDevice3: // Used for center
			{
				current_video->hidden->iPutOffset = TPoint(current_video->screen->w/2-dspSize.iWidth/2, current_video->screen->h/2-dspSize.iHeight/2);
			}
			break;
		}
	...
	if (current_video->hidden->iPutOffset.iX+dspSize.iWidth> current_video->screen->w)
		current_video->hidden->iPutOffset.iX = current_video->screen->w-dspSize.iWidth;

	if (current_video->hidden->iPutOffset.iY+dspSize.iHeight> current_video->screen->h)
...
void ToggleFNMode()
{
	...
	if (current_video->hidden->iFNModeOn)
		{
		current_video->hidden->iStatusChar = _L("FN");
		}
	else
		{
...
TKeyResponse CEBasicAppUi::HandleControlKeyKeysL(const TKeyEvent& aKeyEvent, TEventCode aType)
{
	if (aType == EEventKeyDown)
	{
		...
		case '4':
			...
			UpdateAndRedrawScreenL();
			SetStatusCharAndTimer((current_video->hidden->iSX0Mode& ESX0Stretched)?_L("<->"):_L("1:1"));
			}
			break;
		case 'r':
		case 'R':
		...
		case '5':
			...
			UpdateAndRedrawScreenL();
			SetStatusCharAndTimer((current_video->hidden->iSX0Mode& ESX0DontInterpolate)?_L("FAST"):_L("NICE"));
			}
			break;
		case 'a':
		case 'A':
...
TBool HandleJoyAndMouseKeys(const TInt aSwitchCode, TBool aDown)
	{
	switch (aSwitchCode)
		{
			...
			case EStdKeyUpArrow:
				...
				if (current_video->hidden->iInputMode == EJoystick)
				{
					...
					{
						if (current_video->hidden->iSX0Mode& ESX0Flipped)
							current_video->hidden->iJoystickStatus[EJoyLEFT] = aDown;
						else
...
void CEBasicAppUi::HandleForegroundEventL(TBool aForeground)
{
	...
	{
		iEikonEnv->VirtualCursor().SetCursorStateL(TEikVirtualCursor::EOff, *iEikonEnv);
