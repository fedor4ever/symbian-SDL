...
void EPOC_CalcStretchFactors(_THIS, TSize /*aTargetSize*/)
{
	...
	if(Private->iHasMouseOrTouch && Private->iVirtualKeyBoardActive && current_video->hidden->iVKBTopLeft)
	{
		if(isToTheSide)
		{
			Private->iVKBOffset = TPoint(Private->iKeyboardRect.Size().iWidth,0);
			Private->iKeyboardRect.Move(TPoint(-Private->iKeyboardRect.iTl.iX,0));
		}
		else
		{
			Private->iVKBOffset = TPoint(0,Private->iKeyboardRect.Size().iHeight);
			Private->iKeyboardRect.Move(TPoint(0, -Private->iKeyboardRect.iTl.iY));
		}
	}
	else
		Private->iVKBOffset = TPoint();
}
...
{
	if(!gViewVisible || !_this->screen->pixels)
		return;
	...
	if(Private->iNeedFullRedraw)
	{
		...
		for(TInt loop = 0; loop < numrects; loop++)
		{
			...
			if(Private->iSX0Mode & ESX0Flipped)
			{
				TRect realRect(i240StartTable[rect.y+rect.h-1],
						i320StartTable[rect.x], i240StartTable[rect.y] + 1, i320StartTable[rect.x + rect.w - 1] + 1);
				Private->iWindowCreator->PutBitmap(Private->EPOC_Bitmap,realRect.iTl,realRect);
			}
			else
			{
				TRect realRect(i240StartTable[rect.y],
						i320StartTable[(319 - (rect.x + rect.w - 1))],
						i240StartTable[rect.y + rect.h - 1] + 1, i320StartTable[319 - rect.x] + 1);
				Private->iWindowCreator->PutBitmap(Private->EPOC_Bitmap,realRect.iTl,realRect);
			}
		}
		Private->iWindowCreator->UpdateScreen();
